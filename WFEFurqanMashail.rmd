...
print(math_operation(5, 2, "abcd"))        
print(math_operation(10, 20, "divide")) 
...
double_mtrx <- apply(mtrx, c(1, 2), function(x) x * 2) #creating function
...
pizza_toppings("cheese", "pepperoni", "mushrooms", "onions")
...
deposit <- function(amount, balance = 500) {
  if (amount < 0) {
    stop("Deposit amount cannot be negative!")  # Throws error with msg
  }
...
sum_matrix <- function(m) {
  ...
  for (i in 1:nrow(m)) {
    ...
    for (j in 1:ncol(m)) {
      ...
      Sys.sleep(0.25)  # Paus
      setTxtProgressBar(pb, (i - 1) * ncol(m) + j)  # Update progress bar
    }
  }
...
# Test the function with mtrx
cat("The sum of the matrix is:", sum_matrix(mtrx), "\n")
```
...
result1 <- try(withdrawal(-50), silent = TRUE)
print(result1)

result2 <- try(withdrawal(250), silent = TRUE)
print(result2)

result3 <- try(withdrawal(50), silent = TRUE)
print(result3)
...
input_validation <- function(value) {
  result <- tryCatch({
    ...
    message("Error: Input data must be numeric.")
    return(NA)
  })
...
count_vowel <- function(input_string) {
  ...
  cat("There are", vowel_count, "vowel(s) in '", input_string, "'\n", sep = "")
}
