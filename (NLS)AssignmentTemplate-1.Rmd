...
scores <- c(95,60,88,73,62, 93, 65, 70, 78, 69, 61, 99, 64, 80)
score_factors<- cut (

      scores,
      breaks=c(0,70,76,100),
      labels=c("Poor", "Good", "Excellent"),right=FALSE)
...
online <- class$modes[class$modes=="Online"]
online
```
...
high_earners <- employees[employees$pay >= 1500, ]
...
m <- matrix(10:18, nrow = 3, byrow = TRUE, dimnames = list(c("A", "B", "C"), c("1", "2", "3")))
v <- as.vector(m)
v
```
